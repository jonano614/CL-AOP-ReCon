...
void CompanionTravel_SetTraveller(ref _NPChar)
{
	...
	for(int iTemp=1; iTemp<=3; iTemp++) // Нужно, чтоб была свободная группа
	{
	...
	AddQuestUserData("CompanionTravel", "sShipInfo", XI_ConvertString(RealShips[sti(sld.Ship.Type)].Basename + "Pre") + " '" + sld.Ship.name + "'");
	AddQuestUserData("CompanionTravel", "sColony", XI_ConvertString("Colony" + sld.CompanionTravel.ToColonyID + "Dat")); //TODO: падеж*/
...
void CompanionTravel_TimerToSetInColony(string sCompanion) // Таймер вышел (т.е. компаньона нужно потавить в колонию или потопить)
{
	...
	{
		Log_TestInfo("Критический баг: не найден один из атрибутов у компаньона-путешественника "+sCompanion+". Немедленно сообщите Warship'у.");
		return;
	}
...
void CompanionTravel_DeleteCompanion(string sID, string sCompanion, bool WaitInColony)
{
	...
	if(WaitInColony)
	{
		Log_TestInfo("Кэпа группы CompanionTravel_"+sCompanion+" слишком долго ждал в колонии и ушёл.");
	}
	else
	{
...
/*void CompanionTravel_SetEatFood(ref _NPC) // TODO del
{

...
int Group_CreateGroup(string sGroupID)
{
	...
	Log_TestInfo("Error: Group_CreateGroup нет места в массиве");
	return -1;
}
...
ref	Group_GetGroupByIndex(int iGroupIndex)
{
    ...
    {
        Log_TestInfo("Error: Group_GetGroupByIndex нет группы");
...
ref	Group_FindOrCreateGroup(string sGroupID)
{
	...
	if (iGroupIndex < 0)
	{
		...
		if (iGroupIndex < 0) 
		{
			...
			Log_TestInfo("Error: Group_FindOrCreateGroup не создалась группа " + sGroupID);
...
ref Group_GetGroupCommander(string sGroupID)
{
	...
	if (!CheckAttribute(rGroup, "MainCharacter") && MOD_BETTATESTMODE == "On")
	{
		...
		Log_TestInfo("ERROR: Group_GetGroupCommander нет коммандера для " + sGroupID);
	}
...
int Group_GetDeadCharactersNumR(ref rGroup)
{
	...
	for (int i=0; i<iNum; i++)
	{
		...
		if (and(CheckAttribute(&Characters[iCharacterIndex], "Ship.Type") && sti(Characters[iCharacterIndex].Ship.Type) == SHIP_NOTUSED, true) || LAi_IsDead(&Characters[iCharacterIndex])) { iDeads++; }  // fix фантомов
	}
	return iDeads;
}
...
bool Group_isDeadR(ref rGroup)
{
	...
	while (true)
	{
		...
		if (and(CheckAttribute(&Characters[iCharacterIndex], "Ship.Type") && sti(Characters[iCharacterIndex].Ship.Type) == SHIP_NOTUSED, true) || LAi_IsDead(&Characters[iCharacterIndex])) { iNumDeadCharacters++; }  // стертый тоже труп
		i++;
	}
...
void Group_CheckTask()
{
    ...
    {
        Log_TestInfo("Error: Group_CheckTask нет группы");
        return;
    }

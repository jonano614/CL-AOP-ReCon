			...
			else
			{
				...
				for(i = 0; i < num; i++)
				{
					...
					{					
						...
						if (bMainCharacterInBox && chr.chr_ai.type.state != "afraid" && !CheckAttribute(loadedlocation, "freeboxes"))
						{
							if(chr.chr_ai.group != LAI_GROUP_PLAYER_OWN || chr.dialog.Filename != "Residence_dialog.c") // Rosarak. Убираем проверку в резиденциях городов ГГ
							{
								...
								//<-- снимаем если есть флаг на таймер ссоры у хозяина - снимаем
								if (chr.sex == "man") chr.greeting = "VOICE\" + VoiceGetLanguage() + "\soldier_arest_1.wav";
								else chr.greeting = "VOICE\" + VoiceGetLanguage() + "\Woman_FackYou.wav";
						...
						else
						{
							if(chr.chr_ai.tmpl == LAI_TMPL_STAY)
							{
								...
								if (LAi_CheckFightMode(pchar) && chr.chr_ai.group != LAI_GROUP_PLAYER_OWN)
								{

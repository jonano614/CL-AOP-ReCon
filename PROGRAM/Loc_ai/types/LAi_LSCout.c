			...
			if(stf(chr.chr_ai.type.look_around) < 0.0)
			{
				...
				if(num > 0)
				{
					...
					{	//если не ГГ, то ближайший нпс (сортировка)				
						...
						if (stf(chrFindNearCharacters[0].dist) < 2.5 && stf(chr.chr_ai.type.notalk) <= 0.0 && !CheckAttribute(GetCharacter(trg), "dontTalkOnDuty"))
						{
							isDialog = true;
						}
					}
...
void LAI_TYPE_LSCout_Goto(aref chr)
{
	...
	{
		string shipLoc = "01,02,04,05,06,07,08,09,10,11,14,15"; //без 12 Веласко и без 16 Сан Габриель
		if (CheckAttribute(pchar, "questTemp.LSC.NarvalDestroyed")) shipLoc += ",16"; //после зачистки Веласко разрешаем там тереться местным
		if (CheckAttribute(pchar, "questTemp.LSC.CasperDestroyed")) shipLoc += ",03,12,13"; //после зачистки Сан Габриеля разрешаем там тереться местным
		newloc = GetRandStr(shipLoc, ",");
	}
...
// Нумерация кораблей:
// 01 - Баттлшип Тартарус   TartarusPrison

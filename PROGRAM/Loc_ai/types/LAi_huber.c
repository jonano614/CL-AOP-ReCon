...
void LAi_type_huber_CharacterUpdate(aref chr, float dltTime)
{
    ...
    if (CheckAttribute(chr, "standUp") && !LAi_IsCapturedLocation) //не при захвате колонии
    {
        ...
        if (iNumEnemy > 0) //есть враги
        {
            ...
            LAi_type_huber_prefight(chr);    //ребрендинг
            LAi_SetWarriorTypeNoGroup(chr);    //и вперёд на врага
            LAi_NPC_GetUp(chr);
...
void LAi_type_huber_prefight(ref chr)
{
...
int LAi_type_huber_FindEnemy(aref chr, int num)
{
    if (num <= 0) return -1;
    int i, idx;
    bool bJail = HasStr(chr.id, "JailOff");
    ...
    if (LAi_grp_playeralarm > 0)
    {
        for (i = 0; i < num; i++)
        {
            ...
            //Rosarak. Fix: коменданты не реагировали на обнажение оружия, если ударить кого-то из квестовых спутников (зека, например)
            if (bJail && LAi_CheckFightMode(&Characters[idx])) return idx;

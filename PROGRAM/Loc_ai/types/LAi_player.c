...
void LAi_type_player_CharacterUpdate(aref chr, float dltTime)
{
	...
	/* TO_DO: REWORK
    if (CheckAttribute(&TEV, "captainInTheLSC") && !CheckCharacterPerk(chr, "Merman")) 
	{
		string sAniName;
		SendMessage(chr, "le", MSG_CHARACTER_GETCURANIMATION, &sAniName);
		if(sAniName == "swim")
		{
			chr.MermanTimer = stf(chr.MermanTimer) - dltTime;
			if (sti(chr.MermanTimer) <= 0.0)
			{
				DeleteAttribute(chr, "MermanTimer");
				chr.actions.swim = "fast swim";
				EndChangeCharacterActions(chr);
				SetCharacterPerk(chr, "Merman");
			}
		}
	}*/
	...
	else chr.chr_ai.type.weapontime = "0";
}

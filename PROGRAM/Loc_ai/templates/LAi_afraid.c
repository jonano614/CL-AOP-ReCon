...
{
	if (LAi_grp_playeralarm == 0 && !LAi_CheckFightMode(&Characters[sti(chr.chr_ai.tmpl.who)]))
	{	//HardCoffee фикс преследования убегающего нпс, если по факту всё спокойно
		chr.chr_ai.tmpl.stopAfraid = its(sti(chr.chr_ai.tmpl.stopAfraid) + 1);
		if (sti(chr.chr_ai.tmpl.stopAfraid) > 6)
		{
			DeleteAttribute(chr, "chr_ai.tmpl.stopAfraid");
			return true;
		}
	}
	else chr.chr_ai.tmpl.stopAfraid = "0";

...
bool LAi_tmpl_SetFight_NoTarget(aref chr)
{
	if(!LAi_tmpl_fight_InitTemplate(chr)) return false;
...
void LAi_tmpl_fight_Stop(aref chr)
{
	...
	if(distToTarget < 4.0)
	{
		...
		CharacterPlayAction(chr, LAi_tmpl_fight_GetNearIdle(IsCharacterWithFists(chr)));
	}else{
		//Дальняя область
		chr.chr_ai.tmpl.stopMode = "far";
		chr.chr_ai.tmpl.time = 3 + rand(20)*0.1;
		chr.chr_ai.tmpl.nostop = 2 + rand(10)*0.1;
		CharacterPlayAction(chr, LAi_tmpl_fight_GetFarIdle(IsCharacterWithFists(chr)));
	}
}
...
string LAi_tmpl_fight_GetNearIdle(bool isFists)
{
	string unarmed = "";
	if (isFists) unarmed = "_u";
	string ani = "fight stand_1" + unarmed;
	if(rand(5) < 2)
	{
		ani = "fight stand_2" + unarmed;
		return ani;
	}
	if(rand(5) < 3)
	{
		ani = "fight stand_3" + unarmed;
		return ani;
	}
	if(rand(5) < 1)
	{
		ani = "attack_round_1" + unarmed;
		return ani;
	}
	if(rand(5) < 1)
	{
		ani = "attack_force_1" + unarmed;
		return ani;
	}
	if(rand(5) < 1)
	{
		ani = "attack_force_3" + unarmed;
		return ani;
	}
	return ani;
}

string LAi_tmpl_fight_GetFarIdle(bool isFists)
{
    string unarmed = "";
 	if (isFists) unarmed = "_u";
	string ani = "fight stand_1" + unarmed;
	if(rand(5) < 1)
	{
		ani = "fight stand_4" + unarmed;
		return ani;
	}
	if(rand(5) < 1)
	{
		ani = "fight stand_5" + unarmed;
		return ani;
	}
	if(rand(5) < 1 && unarmed == "")
	{

...
void CalculateEndSiegeCity()
{
	...
	totalInfo = XI_ConvertString("ColonyLevel4") + " " + GetCityName(sCity) + " " + XI_ConvertString("ColonyState1") + " " + XI_ConvertString(sTemp + "Gen") + ".";
...
void CalculateSiegeCity()
{
	...
	{
	    totalInfo = XI_ConvertString("ColonyState2") + " "+XI_ConvertString(sTemp + "Gen")+" " +XI_ConvertString("ColonyState3")+" "+GetCityName(sCity)+". "+ XI_ConvertString("ColonyState4")+FindDaysString(iDay)+XI_ConvertString("ColonyState5");
	}
	else
	{
		totalInfo = XI_ConvertString("ColonyState6") + " "+XI_ConvertString(sTemp + "Gen")+" " +XI_ConvertString("ColonyState7")+" "+GetCityName(sCity)+". "+ XI_ConvertString("ColonyState4")+FindDaysString(iDay)+XI_ConvertString("ColonyState5");
	}
    SetNewGroupPicture("Nat1", "NATIONS",  sTemp);
    SetFormatedText("INFO_TEXT",totalInfo);
}
...
void CalculateCapturedCity()
{
    ...
    totalInfo = XI_ConvertString("ColonyState8") + XI_ConvertString("Colony" + sCity) + XI_ConvertString("ColonyState9");
